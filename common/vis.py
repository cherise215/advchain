...
def plot_noise(data,ax=None,font_size=12, title="rand noise"):
    ...
    '''
    if ax is not None:
        ax.imshow(data,cmap='RdBu_r',interpolation='none',vmin=-np.max(np.array(data)))
        ax.set_title(title,size=font_size,weight='bold')
        ax.set_axis_off()
        ax.grid(False)
    else:
        plt.imshow(data,cmap='RdBu_r',interpolation='none',vmin=-np.max(np.array(data)))
        plt.title(title,size=font_size,weight='bold')
        plt.axis('off')
...
def plot_warped_grid( dvf, ax=None,bg_img=None, interval=3, title="$\mathcal{T}_\phi$", fontsize=20, linewidth=0.5,show=True):
    ...
    if show:
        if ax is not None: 
            ...
            # ax.plot(0,10,'o')
        else:img = plt.imshow(background, cmap='gray')

    # plt: (0,0) is on the left upper corner.
    if ax is not None:
        for i in range(xx.shape[0]):
            ax.plot((new_grid_X[i, :]), (new_grid_Y[i, :]),**kwargs)  # each draws a horizontal line
        for i in range(xx.shape[1]):
            ax.plot(new_grid_X[:, i], (new_grid_Y[:, i]), **kwargs)  # each draws a vertical line
        ax.set_title(title, fontsize=fontsize,weight='bold')
        ax.axis('off')
    else:
        for i in range(xx.shape[0]):
            plt.plot((new_grid_X[i, :]), (new_grid_Y[i, :]),**kwargs)  # each draws a horizontal line
        for i in range(xx.shape[1]):
            plt.plot(new_grid_X[:, i], (new_grid_Y[:, i]), **kwargs)  # each draws a vertical line
